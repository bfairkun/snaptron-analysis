...
Maybe that plot would look better if I only included recurrent mutations (n>1) to get rid of some noise.
...
MutationList %>%
  # filter(Cohort %in% c("LAML", "SKCM", "BRCA", "BLCA", "THYM", "UCEC", "UVM")) %>%
  filter(Hugo_Symbol %in% c("SF3B1")) %>%
  group_by(Protein_position) %>% add_tally() %>% ungroup() %>%
  filter(n>1) %>%
  ggplot(aes(as.numeric(Protein_position))) +
  geom_histogram(aes(fill=Cohort_simplified), bins=50) +
...
# This plot is for a lab presentation
ggplot(ToPlot, aes(x=PC1, y=PC2, color=CancerType, label=Mutant_aa)) +
  geom_text(alpha=1, size=4) +
  geom_vline(xintercept=-5, linetype="dashed", 
                color = "black", size=1) +
