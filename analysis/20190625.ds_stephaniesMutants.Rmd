...
effect.sizes <- read.table(gzfile("../output/snakemake_out_sf3b1_k700e_e622d_ds/leafcutter.ds.effect_sizes.txt.gz"), header=T)

cluster.significance <- fread(paste0("gunzip -c ", "../output/snakemake_out_sf3b1_k700e_e622d_ds/leafcutter.ds.cluster_sig.txt.gz"), sep="\t", header=TRUE, data.table=FALSE)
...
First I will look at PC space based on these set of junctions, and see if perhaps sf3b1mut-wt separates in any of the top PCs. Additionally, another exersize to do is to look within a single tissue type at the same set of junctions identified by this meta-tissue analysis.
